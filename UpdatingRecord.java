...
public class UpdatingRecord {
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            PreparedStatement fetch = con.prepareStatement("SELECT * FROM BCA3 WHERE empid=?");
            fetch.setInt(1, id);
            ...
            if (!rs.next()) {
                ...
                PreparedStatement ps = con.prepareStatement("UPDATE BCA3 SET empname=?, address=?, mobileno=?, designation=?, department=?, salary=? WHERE empid=?");
                ps.setString(1, name);
